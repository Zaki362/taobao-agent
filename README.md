...
仓库采用“同一共享实现、两个独立应用”的结构：正式产品仍从仓库根目录构建；公开体验由 `apps/public-demo` 独立静态构建，只导出 `/`、`/demo` 和 404。Demo 直接复用正式产品的需求确认、购物规划、搜索进度、推荐、加购与购物清单组件，不维护第二套产品交互；正式产品本身不再暴露 `/demo` 路由。
